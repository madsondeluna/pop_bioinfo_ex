````markdown
# Procedimento Operacional Padrão (POP) – [Título da Técnica]
...
**Aprovado por:** [Nome da chefia, se aplicável]
...
- Arquivo(s) de entrada: [ex: `sample.vcf.gz`]
- Formato: [ex: VCF v4.2, tabular, FASTA]
- Pré-processamento necessário: [ex: compressão com bgzip, indexação com tabix]
...
* Critérios para validação da execução: \[ex: número de variantes ≈ ao do VCF original]
...
* Verificação de colunas-chave: \[ex: `SIFT_pred`, `PolyPhen2_HDIV_pred`]
...
| Arquivo `.csv` vazio        | Arquivo VCF com formatação incorreta | Validar VCF com `vcf-validator`       |
...
| dd/mm/aaaa | v1.1   | Revisão de banco de dados usado | \[Nome]       |
